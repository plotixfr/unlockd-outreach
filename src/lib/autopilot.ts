...
// Sonnet generating a 5-email FR/NL sequence (≈2-3k output tokens) routinely
// needs 25-60s — the old 22s cap aborted ~half of generations mid-flight
// (the "returned 0 emails" redrive failures). 90s default now that the
// per-run budget is 240s; env-tunable without redeploy.
const GEN_TIMEOUT_MS = Number(process.env.AUTOPILOT_GEN_TIMEOUT_MS ?? 90_000);
...
export interface GenerateResult {
  count: number;
  /** Distinct failure signature when count === 0 — drives lastError so a
   * timeout abort is distinguishable from an empty/parse failure. */
  failure: string | null;
}
...
export async function generateEmailsInline(
  ...
  if (!apiKey) return { count: 0, failure: "no ANTHROPIC_API_KEY" };
  const anthropic = new Anthropic({ apiKey });

  const prospect = await prisma.prospect.findUnique({ where: { id: prospectId } });
  if (!prospect) return { count: 0, failure: "prospect not found" };
  ...
  try {
    ...
    if (!Array.isArray(parsed) || parsed.length === 0) {
      return { count: 0, failure: "parsed empty/non-array" };
    }
  } catch {
    return { count: 0, failure: "json parse failed" };
  }
  ...
  return { count: parsed.length, failure: null };
}
...
async function processPlace(
  ...
  if (generated.count === 0) {
    ...
    // The signature distinguishes a timeout abort from an empty generation.
    await prisma.prospect.update({
      where: { id: prospect.id },
      data: { lastError: `generate: ${generated.failure ?? "unknown failure"}` },
    });
    return { status: "qualified", reason: `email generation failed (${generated.failure})` };
  }
